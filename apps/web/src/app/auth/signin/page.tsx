...
export default function SignInPage() {
  ...
  const handleOtpLogin = async (e: React.FormEvent) => {
    ...
    try {
      const response = await apiClient.requestLoginOtp(formData.email);
      
      if (response.success) {
        setStep(2);
      } else {
        setErrors({ general: response.error || 'Failed to send login code' });
      }
    } catch (error) {
      setErrors({ general: 'Network error. Please try again.' });
    } finally {
      ...
      <div className="flex items-center justify-center px-4 pt-24 pb-12">
        <div className="max-w-md w-full">
          ...
          >
            ...
            <p className="text-xl text-gray-600 leading-relaxed">Sign in to your student community account</p>
          </motion.div>
